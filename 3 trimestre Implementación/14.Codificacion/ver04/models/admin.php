...
public function movimientos_query(){
    ...
    public function personas(){
        try{
            $result = parent::connection()->prepare("SELECT * FROM personas INNER JOIN estados ON personas.estado = estados.id_estado");
            $result->execute();
            return $result->fetchAll();
        }catch(Exception $e){
                die("No se logro realizar la consulta" . $e->getMessage());
        ...
        public function crear_persona($data){
            try{
                $result = parent::connection()->prepare("INSERT INTO personas (PersoNombres, Correo, TipoDoc, documento, Direccion, Telefono, estado) VALUES (?,?,?,?,?,?,1)");
        ...
        public function act_state_persona(){
            try{
            $result = parent::connection()->prepare("UPDATE personas SET estado = 1 WHERE id_persona = ?");
            $result->bindParam(1,$_GET['id_persona'], PDO::PARAM_STR);
            $result->execute();
            }catch(Exception $e){
                die("No se logro realizar el registro" . $e->getMessage());
        ...
        public function inac_state_persona(){
            try{
            $result = parent::connection()->prepare("UPDATE personas SET estado = 2 WHERE id_persona = ?");
            $result->bindParam(1,$_GET['id_persona'], PDO::PARAM_STR);
            $result->execute();
            }catch(Exception $e){
                die("No se logro realizar el registro" . $e->getMessage());
